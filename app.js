...
function collectVariants(payload, query, limit = 6) {
  const items = [];
  const seen = new Set();
...
async function runTranslator(query, direction) {
  ...
  try {
    ...
    // If quality seems poor, try via EN pivot (closer to Google-like robustness)
    if (isBadTranslation(query, best)) {
      const pivotPayload = await fetchMyMemoryPayload(query, sourceLang, fallbackPivot);
      const pivot = pickBestTranslation(pivotPayload, query);

      if (!isBadTranslation(query, pivot)) {
        const targetPayload = await fetchMyMemoryPayload(pivot, fallbackPivot, targetLang);
        const pivotBest = pickBestTranslation(targetPayload, pivot);
        const pivotVariants = collectVariants(targetPayload, pivot);

        if (!isBadTranslation(query, pivotBest)) best = pivotBest;
        if (pivotVariants.length) variants = [...new Set([...variants, ...pivotVariants])].slice(0, 8);
      }
    }
  ...
  if (local.length) {
    ...
    return {
      source: 'local',
      text: direction === 'ru-la' ? primary.latin : primary.meaning,
...
function renderStatsDashboard() {
  ...
  const easy = state.words.filter((w) => w.difficulty === 'easy').length;
  const medium = state.words.filter((w) => w.difficulty === 'medium').length;
  const hard = state.words.filter((w) => w.difficulty === 'hard').length;

  const data = [
    { label: 'Выучено', value: learned, max: total },
    { label: 'Лёгкие слова', value: easy, max: total },
    { label: 'Средние слова', value: medium, max: total },
    { label: 'Сложные слова', value: hard, max: total },
    { label: 'Верные ответы', value: state.stats.correctAnswers, max: Math.max(1, state.stats.quizzesTaken || state.stats.correctAnswers || 1) }
  ];
