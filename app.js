...
function applyAccent(hex) {
  const safe = /^#[0-9a-fA-F]{6}$/.test(hex) ? hex : '#50C878';
...
function initAppearance() {
  ...
  const savedAccent = localStorage.getItem(ACCENT_KEY) || '#50C878';
...
function runTranslator(query, direction) {
  const normalizedQuery = normalize(query);
  if (!normalizedQuery) return { exact: [], fuzzy: [] };

  if (direction === 'ru-la') {
    const exact = state.words.filter((w) => normalize(w.meaning) === normalizedQuery);
    const fuzzy = state.words.filter((w) => normalize(w.meaning).includes(normalizedQuery) || normalize(w.latin).includes(normalizedQuery));
    return { exact, fuzzy };
  }

  const exact = state.words.filter((w) => normalize(w.latin) === normalizedQuery);
  const fuzzy = state.words.filter((w) => normalize(w.latin).includes(normalizedQuery) || normalize(w.meaning).includes(normalizedQuery));
  return { exact, fuzzy };
}

function renderTranslatorResult(matches, direction) {
  if (!el('translatorResult') || !el('translatorHints')) return;
  const result = el('translatorResult');
  const hints = el('translatorHints');
  hints.innerHTML = '';

  if (!state.words.length) {
    result.classList.add('muted');
    result.textContent = 'Словарь пуст. Добавь слова на странице «Слова». ';
    return;
  }

  const top = matches.exact.length ? matches.exact : matches.fuzzy;
  if (!top.length) {
    result.classList.add('muted');
    result.textContent = 'Совпадений не найдено в личном словаре.';
    return;
  }

  const primary = top[0];
  result.classList.remove('muted');
  result.textContent = direction === 'ru-la'
    ? `${primary.meaning} → ${primary.latin}`
    : `${primary.latin} → ${primary.meaning}`;

  top.slice(1, 6).forEach((w) => {
    const li = document.createElement('li');
    li.textContent = `${w.latin} — ${w.meaning}`;
  ...
  words.forEach((word) => {
    ...
    actions.append(
      makeBtn(word.learned ? 'В процессе' : 'Выучено', 'secondary-btn', () => {
        ...
        renderWordList();
        renderStatsIfExists();
  renderTranslatorResult({ exact: [], fuzzy: [] }, "la-ru");
      }),
...
function wireEvents() {
  ...
  el('translatorForm')?.addEventListener('submit', (e) => {
    e.preventDefault();
    const direction = el('translatorDirection').value;
    const query = el('translatorInput').value;
    const matches = runTranslator(query, direction);
    renderTranslatorResult(matches, direction);
  });
